'''This file simplifies the previous file's code, including
removing the neural gate class (replaced with neural layer)
and using units to stores matrices, instead of matrices to
store units'''
...
matrix, and accept an array of arguments'''

activation_functions = {
    'ReLu': {
        'activator': lambda x, *args: np.maximum(x*args[0], x),
        'derivative': lambda x, *args: np.array([[args[0] if num < 0 else 1 for num in row] for row in x])
    },
    'tanh': {
...
class CostGate:
    ...
    neural network. Sets the gradients to being backpropogation'''

    def computeCost(self, predicted_labels, true_labels):
        '''Computes the mean cost of a network's predicted labels against the true labels'''
        
        return (0.5 * sum((predicted_labels.values - true_labels)**2))/predicted_labels.values.shape[0]
        
    def set_gradients(self, predicted_labels, true_labels, learning_rate):
        '''Sets the upper level gradients of a network's output according to the
        derivative of the cost function with respect to the predicted labels.'''

        d_predicted_labels = (true_labels - predicted_labels.values) * learning_rate
        predicted_labels.grads = d_predicted_labels
        
        #print(predicted_labels.values)
        #print(true_labels)
        #print(d_predicted_labels)

    def classification_percent(self, predicted_labels, true_labels):
        '''Computes the percent of examples that the network classifies correctly
        Correct classification means the index of the highest output matches the true labels'''

        truth_table = np.argmax(predicted_labels.values, axis = 1) == np.argmax(true_labels, axis = 1)
        return (np.count_nonzero(truth_table))/len(truth_table) * 100

class NeuralLayer:
    '''Basic building block of the neural network. Matrix multiplication
    used for feed forward, and back propogation'''

    def __init__(self, parameters, activation, activator_params = None, learning_rate = 1):
    ...
    @classmethod
    def new(cls, inputs, outputs, activation, activator_params=None, learning_rate=1, weight_multiplier=None):
        '''Initilizes a new radom neural layer'''
        
        # Estimates optimal weight initilization range to prevent enourmous outputs
    ...
    def forward(self, x):
        ...
        self.output = Unit(self.activation(self.z, *self.activator_params), np.zeros(self.z.shape))
        
        return self.output

    def backward(self):
        '''Computes the gradients with respect to z, a, and x, and stores them accordingly.
        The parameters are updated immediatly, since they don't affect backprop
        Only the layer's own parameters are modified by the layer's learning rate.'''
        
        dz = np.multiply(self.activation_prime(self.z, *self.activator_params), self.output.grads)
...
class NeuralNetwork:
    '''Stores multiple neural layers to form a fully connected neural network'''
    ...
    def from_parameters(cls, parameters, activation, activator_params=None):
        '''Creates a neural network from a list of parameters by manually creating the
        neural layers'''
        
        layers = [NeuralLayer(parameters[i], activation, activator_params) for i in range(len(parameters))]
        
        return cls(layers)
    
    @classmethod
    def new(cls, description, activation, activator_params=None):
        '''Creates a new neural network by automatically creating
        a series of new random neural layers'''
        
        layers = [NeuralLayer.new(description[i], description[i + 1], activation, activator_params) for i in range(len(description) - 1)]
    
        return cls(layers)

    def forward(self, x):
        '''Automates feed forward through every layers'''
    ...
    def backward(self):
        '''Automates backpropogation through each layers. Note that
        the upper-level gradients must be set by something (e.g. a cost gate)
        for gradients to be non-zero.'''
...
class Trainer:
    ...
    def __init__(self, network, training_data, validation_data, classification = False):
        '''Stores the network, a CostGate, and all the data necessary for training
        and validation. Also stores network type to run percent success functions
        ...
        self.DEFAULT_TRAINING_SETTINGS = {
            ...
            'epoch_blocks': 1,
            'learning_rate': 0.5 # TODO: estmiate appropriate learning rate
        }
    ...
    def train(self, training_settings = {}, halt_settings = {}):
        '''Trains the neural network with the specifide conditions, and then reports
        on the results of training'''

        self.training_settings = set_defaults(training_settings, self.DEFAULT_TRAINING_SETTINGS)
        self.halt_settings = set_defaults(halt_settings, self.DEFAULT_HALT_SETTINGS)
        ...
        while self.epochs < self.halt_settings['max_epochs']:
            for j in range(self.training_settings['epoch_blocks']):
                self.trainEpoch(self.training_settings['batch_size'], self.training_settings['learning_rate'])
                self.epochs += 1
            ...
            if self.validation_cost <= self.halt_settings['min_validation_cost']:
                self.exitMessage('Validation cost threshold acheived')
                return

            if self.classification and self.percent > self.halt_settings['min_classification_percent']:
    ...
    def trainEpoch(self, batch_size, learning_rate):
        ...
        while len(shuffled_inputs) > 0:
            inputs, shuffled_inputs = shuffled_inputs[:batch_size], shuffled_inputs[batch_size:]
            labels, shuffled_labels = shuffled_labels[:batch_size], shuffled_labels[batch_size:]
    ...
    def set_diagnostics(self):
        '''Stores variables related to the performance of the network'''
        
        self.network.forward(self.validation_data['inputs'])
        self.validation_cost = np.sum(self.costgate.computeCost(self.network.output, self.validation_data['labels']))
        self.time = time() - self.start
        
        if self.classification:
            self.percent = self.costgate.classification_percent(self.network.output, self.validation_data['labels'])

        if self.validation_cost < self.best_validation_cost:
            self.best_validate_params = deepcopy(self.network.give_parameters())
    ...
    def print_diagnostics(self, message):
        ...
        print(message, end = '')
        print('Validation Cost: ' + str(self.validation_cost), end = ' ')
        
        if self.classification:
            print('Validation Percent: ' + str(round(self.percent, 3)) + '%', end = '')
    ...
    def exitMessage(self, message):
        ...
        print(str(self.epochs) + ' epochs/' + str(round(self.time, 3)) + ' seconds\n')
        ...
        self.network.set_parameters(self.best_validate_params)
        self.set_diagnostics()
...
class Preprocessor:
    ...
    def from_data(cls, data_sample, with_bias = False):
        '''Calculates all parameters necessary for Preprocessor automatically'''
        ...
        for key in data_sample:
            stats[key] = {
                'mean': np.mean(data_sample[key], axis = 0),
                'std': np.std(data_sample[key], axis = 0)
            }
            
            data_sample[key] = cls.normalize(None, data_sample[key], stats[key]['mean'], stats[key]['std'])
                
            stats[key]['scale_factor'] = np.max(np.abs(data_sample[key]))
    ...
    def transform_inputs(self, inputs):
        ...
        inputs = self.normalize(inputs, self.input_stats['mean'], self.input_stats['std'])
    ...
    def transform_labels(self, labels):
        '''Applys z-score  normaliation and scaling as well,
        but for labels'''
        
        labels = self.normalize(labels, self.label_stats['mean'], self.label_stats['std'])
        labels = self.scale(labels, self.label_stats['scale_factor'])
        
        return labels
    
    def transform_data(self, data):
        '''Conveinvnce function that transforms both inputs and labels'''
    ...
    def normalize(self, data, mean, std):
        ...
        if (std == 0).any():
            ...
            print('WARNING: ' + str(zeros) + ' features have a standard deviation of 0. Please remove those features.')
...
def load_network(name):
    ...
    network = NeuralNetwork.from_parameters(parameters)
    
    return network

def save_preprocessor(preprocessor, name):
    '''Basic function that stores preprocessor stats
    in a numpy array that is saved'''
...
if __name__ == '__main__':
    ...
    NN = NeuralNetwork.new([3, 3, 2], 'tanh')

    data = P.transform_data(data)
    
    trainer = Trainer(NN, data, data, True)

    trainer.train({'learning_rate': 0.6, 'epoch_blocks': 1}, {'min_validation_cost':0.01, 'max_epochs': 1000})
